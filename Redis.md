...
| 区别     | 跳跃表                                                     | 红黑树                                                       |
| -------- | ---------------------------------------------------------- | ------------------------------------------------------------ |
| 内存     | 小（redis 跳表平均 1.33 个指针）                           | 大（每个节点包含左右子树两个指针）                           |
| 效率     | O(logn)                                                    | O(logn)                                                      |
| 存储方式 | 有序存储                                                   | 有序存储                                                     |
| 代码实现 | 易（插入删除只需修改相邻节点的指针）                       | 难（为了保持树的平衡，需要做很多操作）                       |
| 范围查找 | 简单（只需要找到最小值节点后，根据第一层指针往后查找即可） | 需要以中序遍历查找                                           |
| 并发操作 | 粒度小（增删改操作只需改变局部节点）                       | 粒度大（增删改操作需要做 reblance 操作，可能会涉及到整棵树） |
...
使用 set 命令来获取锁，并设置该锁的超时时间（如果不设置超时时间，若以获取锁的某个进程挂了，则这个锁将会一直存在）。使用 lua 脚本来释放锁。  
参数解释：

> key: 使用 key 来当锁，因为 key 是唯一的。
> value: request_id，因为要满足可靠性时，通过给 value 设置 request_id，我们就知道这把锁是哪个请求的了，在解锁的时候就可以有依据。request_id 可以使用 UUID/clientID 等。
> EX|PX: 设置过期时间，意思是我们要给这个 key 加一个过期的设置（EX 单位为秒，PX 单位为毫秒）。
> NX|XX：这个参数我们填的是 NX，意思是 SET IF NOT EXIST，即当 key 不存在时，我们进行 set 操作；若 key 已经存在，则不做任何操作。
...
基于以上几个缺陷，redis 官方推荐使用 RedLock 实现分布式锁。
